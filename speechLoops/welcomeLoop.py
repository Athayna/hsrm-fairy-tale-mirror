...
class WelcomeLoop(SpeechLoop):
    ...
    def play(self) -> None:
        ...
        if (datetime.datetime.now().hour < 10):
            ...
            self.handler.result = ""
        elif (datetime.datetime.now().hour >= 12 and datetime.datetime.now().hour < 14) and datetime.date.isoweekday(datetime.datetime.now()) in range(1, 5):
            if self.handler.user.school:
                ...
                if any(x in self.handler.result for x in ("naja", "schlecht", "blöd", "doof", "nicht")):
                    self.handler.result = ""
                ...
                elif any(x in self.handler.result for x in ("gut", "toll", "schön", "super", "oke")):
                    self.handler.result = ""
                ...
                if any(x in self.handler.result for x in ("ja", "klar", "genau", "schon")):
                    ...
                    while(1):
                        ...
                        if not self.handler.result:
                            return
                        if any(x in self.handler.result for x in ("nein", "falsch", "nö", "nicht")):
                            self.handler.result = ""
                            self.speak_text("Puh, dann hab ich mich doch nicht geirrt. Super, dass du zur Schule gehst!", watchListSkip)
                            break
                        elif any(x in self.handler.result for x in ("ja", "genau", "richtig", "klar", "sicher")):
                            self.handler.result = ""
